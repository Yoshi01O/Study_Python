...
### データ型一覧
```Python
文字列型　  :str()      #文字列型　　:文字、文字列
整数型　　  :int()      #整数型　　　:小数点を含まない数値
浮動小数点型：float()   #浮動小数点型：小数点を含む数値
日付型　　  :datetime() #日付型　　　　:日付
ブール型　　：bool()    #ブール型　　：TrueやFalce
配列型　　　：list()    #配列型　　　：複数の要素を[]で囲んだデータ。","で区切る。要素の変更不可。
タプル型　　：tuple()   #タプル型　　：複数の要素を()で囲んだデータ。","で区切る。要素の変更可能。<br>
辞書型　　　：dict()    #辞書型　　　：複数の要素を{}で囲んだデータ。","で区切る。要素の変更可能
